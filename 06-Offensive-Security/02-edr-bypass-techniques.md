...
**Repository**: https://github.com/TwoSevenOneT/EDR-Freeze

A user-mode Windows utility that exploits the WerFaultSecure vulnerability to temporarily suspend EDR and antimalware processes without requiring vulnerable drivers.

##### Technical Details

**Core Mechanism:**
- Exploits WerFaultSecure (Windows Error Reporting) component vulnerability
- Achieves process suspension from unprivileged user-mode context
- Includes PPL (Protected Process Light) bypass functionality
- No BYOVD (Bring Your Own Vulnerable Driver) required
- Tested on Windows 11 24H2

**Key Features:**
- User-mode operation (no kernel drivers needed)
- Process suspension via WerFaultSecure vulnerability
- PPL bypass capability per source files
- Configurable suspension duration
- Integration with external scripts/payloads
...
**Example:**
```cmd
# Suspend process 1234 for 10 seconds
EDR-Freeze.exe 1234 10000
```

**Best Practices:**
- Use minimal sleep durations (1-3 seconds)
- Execute payloads immediately after suspension
- Embed target code directly into tool for better reliability
- Avoid extended suspension periods (GUI may become unresponsive)

##### Detection & Defense

**Detection Methods:**
- Monitor for EDR-Freeze.exe execution
- Detect unusual process suspension patterns
- Alert on WerFaultSecure API abuse
- Track PPL bypass attempts
- Monitor Event ID 1001 (Windows Error Reporting) with unusual source processes
- Alert on `PssSuspendProcess` / `PssSuspendThread` API calls combined with `OpenProcess` targeting EDR process IDs

**Indicators of Compromise:**
- EDR-Freeze.exe file hash
- WerFaultSecure abuse patterns
- Sudden EDR process suspensions
- PPL bypass telemetry

**Mitigation:**
- Update to latest Windows patches (CVE checks)
- Enable PPL for critical security processes
- Monitor WER event logs for anomalies
- Deploy behavioral detection for process suspension abuse
- Implement kernel callbacks for process/thread suspension monitoring

**MITRE ATT&CK**: T1562.001 (Impair Defenses: Disable or Modify Tools)

**Requirements:**
- Windows 11 24H2 (tested; likely works on earlier builds)
- User-mode execution privileges
- Target process PID knowledge
- C++ runtime

---

#### EDRSilencer - Network-Based EDR Blocking

**Repository**: https://github.com/netero1010/EDRSilencer

A Windows utility that prevents EDR agents from transmitting telemetry by intercepting and blocking their outbound network communications using Windows Filtering Platform (WFP).

##### Technical Architecture

**Core Technology:**
- Leverages Windows Filtering Platform (WFP) for kernel-level network filtering
- Custom `FwpmGetAppIdFromFileName0` implementation bypasses minifilter-based EDR protections
- Avoids calling `CreateFileW` internally to evade EDR file access controls
- Dual-stack filtering (IPv4 and IPv6 simultaneously)
- Compatible with in-memory execution (e.g., BruteRatel's memexec)

**How It Works:**
1. Detects running EDR processes or accepts manual process path
2. Creates WFP filters to block all outbound traffic from target processes
3. EDR agents continue running but cannot send alerts/telemetry to command servers
4. Maintains filters until manually removed

##### Supported EDR Platforms

EDRSilencer includes detection signatures for **16 major EDR vendors**:

| Vendor | Products Targeted |
|--------|------------------|
| Microsoft | Defender for Endpoint, Windows Defender |
| Elastic | Elastic EDR |
| Trellix | Trellix EDR |
| Qualys | Qualys EDR |
...
| Carbon Black | CB Defense, CB Response |
| Tanium | Tanium EDR |
| Palo Alto Networks | Cortex XDR |
| FortiEDR | FortiEDR |
| Cisco | Cisco AMP, Secure Endpoint |
...
```cmd
# Auto-detect and block all running EDR processes
EDRSilencer.exe blockedr

# Block specific process by full path
EDRSilencer.exe block "C:\Program Files\Defender\MsSense.exe"

# Remove all active WFP filters
EDRSilencer.exe unblockall

# Remove specific filter by ID
EDRSilencer.exe unblock <FilterID>

# Enumerate running EDR processes
EDRSilencer.exe enumerate
```

**Example Workflow:**
```cmd
# 1. Enumerate EDR processes
EDRSilencer.exe enumerate

# 2. Block all detected EDRs
EDRSilencer.exe blockedr

# 3. Perform red team operations
# ... malicious activity here ...

# 4. Cleanup - remove filters
EDRSilencer.exe unblockall
```
...
**Dependencies:**
- Windows Filtering Platform library (`fwpuclnt.dll`)
- Administrative privileges (required for WFP filter creation)
- Windows 10/Server 2016 or later

##### Detection & Defense

**Observable Artifacts:**
- New WFP filter creation events in Windows Event logs
- Modified network filtering rules (visible via `netsh wfp show filters`)
- Unsigned executable running with administrative privileges
- Process imports include `fwpuclnt.dll`
- Network connectivity loss from EDR processes

**Detection Queries:**

```powershell
# Check for EDRSilencer-related WFP filters
netsh wfp show filters | Select-String -Pattern "EDR|Defender|Sentinel"

# Monitor WFP filter creation
Get-WinEvent -LogName "Microsoft-Windows-Windows Firewall With Advanced Security/Firewall" |
  Where-Object { $_.Id -eq 2004 -or $_.Id -eq 2005 }

# Check for processes loading fwpuclnt.dll
Get-Process | ForEach-Object {
  $_.Modules | Where-Object { $_.ModuleName -eq "fwpuclnt.dll" }
}
```

**Sysmon Detection:**
- Event ID 3 (Network Connection): EDR process network failures
- Event ID 7 (Image Loaded): fwpuclnt.dll loaded by suspicious process
- Event ID 1 (Process Create): EDRSilencer.exe execution

**Sigma Rule Concept:**
```yaml
title: EDRSilencer WFP Filter Creation
status: experimental
description: Detects potential EDRSilencer usage via WFP filter manipulation
logsource:
  product: windows
  service: firewall-as
detection:
  selection:
    EventID:
      ...
      - 2005  # Rule Modified
  filter:
    TargetApplication|contains:
      - 'MsSense.exe'
      - 'SentinelAgent.exe'
      - 'elastic-endpoint.exe'
  condition: selection and filter
falsepositives:
  - Legitimate network filtering software
level: high
tags:
...
1. **Preventive Controls:**
   - Enable WFP filter auditing
   - Restrict administrative privileges
   - Deploy application whitelisting (only allow signed network tools)
   - Monitor for unsigned executables with network filtering capabilities

2. **Detective Controls:**
   - Alert on WFP filter modifications targeting security processes
   - Monitor EDR agent network connectivity (heartbeat checks)
   - Detect `fwpuclnt.dll` loading by non-system processes
   - Correlate EDR telemetry gaps with WFP filter events

3. **Response Actions:**
   - Automatically remove unauthorized WFP filters
   - Isolate hosts exhibiting EDR communication failures
   - Investigate processes with WFP capabilities
   - Force EDR agent reinstallation if tampering detected

**MITRE ATT&CK**: T1562.001 (Impair Defenses: Disable or Modify Tools), T1562.004 (Impair Defenses: Disable or Modify System Firewall)

**OpSec Considerations:**
- Requires administrative privileges (high detection risk)
- Creates forensic artifacts in WFP logs
- EDR telemetry gap is noticeable to SOC teams
- Better suited for short-duration operations
- Consider combining with log tampering techniques

**Advantages:**
- EDR processes remain running (less suspicious than termination)
- Blocks cloud-based detection/response
- Works against multiple EDR vendors
- No kernel driver manipulation required

**Limitations:**
- Requires admin rights
- Highly detectable via WFP logs
- EDR local components may still detect on-host activity
- Does not prevent kernel-level monitoring (ETW, kernel callbacks)
- Network isolation may trigger EDR alerting mechanisms
